d3.csv('total_fat_ind.csv').then(data => {
    ...
    let chart = BoxPlot(data, {
        x: d => d.serving_size,
        y: d => d.amount,
        xLabel: "Industry Avg. Serving Size →",
        yLabel: "↑ Industry Avg. Amount (G)",
        width: 600,
        height: 500
      });

      document.getElementById("boxplot_ind").appendChild(chart);
    });
...
function BoxPlot(data, {
    ...
    fill = "#8c96c6", // fill color of boxes
    ...
    if (xDomain === undefined) xDomain = [d3.min(B, d => d.x0)-5, d3.max(B, d => d.x1)];
    if (yDomain === undefined) yDomain = [d3.min(B, d => d.range[0]), 38];
